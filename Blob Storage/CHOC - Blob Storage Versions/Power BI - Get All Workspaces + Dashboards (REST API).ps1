...
#DESCRIPTION: Extract all workspaces + dashboards via REST API and service principal.
    ...
    $ApiUri = '/admin/groups?$top=5000&' + '$expand=dashboards&'+ '$filter=state eq' + " 'Active'" + ' and type eq' + " 'Workspace'"

    #File name for temporary staging file.
    $File = "Workspaces + Dashboards Staging.csv" 

    #File name to be saved in Blob Storage.
    $FileSave = "Workspaces + Dashboards Final.csv"
